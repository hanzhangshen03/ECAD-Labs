...
if {$sixtyfour == "-1"} {
	set ARCH "-m32"
        set CC "i686-linux-gnu-gcc"
} else {
	set ARCH "-m64"
        set CC "x86_64-linux-gnu-gcc"
}

# ... and compile the Clarvi Avalon emulator as an appropriate kind of shared library
exec $CC -shared $ARCH -fPIC -I $MODEL_TECH/../include/ -o clarvi_avalon_external.so clarvi_avalon_external.c
...
if {$argc > 0} {
    vsim  -sv_lib clarvi_avalon_external work.clarvi_sim -GINIT_FILE=$1 -t ns -voptargs=+acc=npr
} else {
    vsim  -sv_lib clarvi_avalon_external work.clarvi_sim -t ns -voptargs=+acc=npr
}
